(function(root,factory){'use strict';if(typeof define==='function'&&define.amd){define(['angular'],factory)}else if(root.hasOwnProperty('angular')){factory(root.angular)}else if(typeof exports==='object'){module.exports=factory(require('angular'))}}(this,function(angular){'use strict';angular=(angular&&angular.module)?angular:window.angular;function isStorageSupported($window,storageType){var supported;try{supported=$window[storageType]}
catch(err){supported=!1}
...
_last$storage=angular.copy($storage)}},$supported:function(){return!!isSupported}},_last$storage,_debounce;$storage.$sync();_last$storage=angular.copy($storage);$rootScope.$watch(function(){_debounce||(_debounce=$timeout($storage.$apply,100,!1))});$window.addEventListener&&$window.addEventListener('storage',function(event){if(!event.key){return}
